...
function onPortalDetailsUpdated(e) {
  const img = document.querySelector(
    '.portal-pictures-image[data-guid="' + e.guid + '"]'
  );
  if (img) {
  ...
  const container = (
    <div style="max-width: 1000px">
      ...
      <div>
        {portals.map((portal) => (
          <img
            src={(portal.options.data.image || defaultImage).replace(
              "http:",
              ""
            )}
...
export default function () {
  ...
  $("#toolbox").append(
    ' <a onclick="window.plugin.portalPictures.showDialog()">Portal pictures</a>'
  );
}
